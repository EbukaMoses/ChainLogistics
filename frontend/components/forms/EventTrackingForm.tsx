...
import { addTrackingEvent } from "@/lib/contract/trackingEvent";
...
export default function EventTrackingForm() {
    ...
    const loadProducts = useCallback(async () => {
        ...
        try {
            const fetched = await getProductsByOwner(publicKey);
            setProducts(fetched.filter((p) => p.active));
        } catch {
            // Non-fatal — product list is a convenience, not a blocker
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const hash = await addTrackingEvent(publicKey, {
                productId,
        ...
        return (
            <div className="bg-white p-8 md:p-12 rounded-3xl shadow border border-gray-100 max-w-2xl mx-auto text-center">
                ...
                <p className="text-gray-600 text-lg mb-8">
                    The tracking event has been immutably recorded on the Stellar blockchain.
                </p>
                <div className="bg-gray-50 rounded-2xl p-6 text-left mb-8 font-mono text-sm max-w-sm mx-auto space-y-3 border border-gray-200 shadow-inner">
                    ...
                    {txHash && (
                        <div className="pt-2 border-t border-gray-200">
                            <span className="text-gray-500 uppercase text-xs tracking-wider block mb-1">Tx Hash</span>
                            <span className="text-gray-700 text-xs break-all">{txHash}</span>
                        </div>
                    )}
                </div>
                <button
                    onClick={() => { setSuccess(false); resetForm(); }}
                    className="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition shadow-md w-full sm:w-auto"
                >
                    Track Another Event
                </button>
            </div>
        ...
        <div className="bg-white p-6 md:p-10 rounded-3xl shadow border border-gray-100 w-full max-w-5xl mx-auto">
            <div className="mb-8 border-b border-gray-100 pb-6">
                ...
                <p className="text-gray-500 mt-2">
                    Record a new step in the product&apos;s journey. Submit to ledger via wallet signature.
                </p>
            </div>

            {walletStatus !== "connected" && (
                <div role="alert" className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm">
                    Connect your wallet to submit tracking events.
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
                <div className="space-y-3">
                    <label htmlFor="product" className="block text-sm font-semibold text-gray-700 uppercase tracking-wide">
                        1. Select Product *
                    </label>
                    <select
                        id="product"
                    ...
                    >
                        <option value="">
                            {loadingProducts
                                ...
                                : products.length === 0 && walletStatus === "connected"
                    ...
                    {walletStatus === "connected" && products.length === 0 && !loadingProducts && (
                        <p className="text-xs text-gray-500">
                            No active products found.{" "}
                            <a href="/register" className="text-indigo-600 underline">Register a product</a> first.
                        </p>
                    )}
                </div>

                <div className="space-y-3">
                    <span className="block text-sm font-semibold text-gray-700 uppercase tracking-wide">2. Select Operation</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                        <LocationInput
                            ...
                            required
                            value={location}
                            onChange={(value) => { setLocation(value); setError(""); }}
                            error={!location && error === "Please fill in all required fields" ? "Location is required" : undefined}
                        />
                    </div>
                    <div className="space-y-3">
                        <label htmlFor="note" className="block text-sm font-semibold text-gray-700 uppercase tracking-wide">
                            4. Remarks (Optional)
                        </label>
                        <input
                            type="text"
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            Additional conditions or remarks. ({note.length}/{EVENT_NOTE_MAX_LEN})
                        </p>
                    </div>
                </div>
                ...
                <div className="pt-8 border-t border-gray-200 mt-10 flex flex-col sm:flex-row items-center justify-end gap-4">
                    ...
                    <button
                        type="submit"
                        disabled={isSubmitting || !productId || !eventType || !location || walletStatus !== "connected"}
                    ...
                    >
                        {isSubmitting ? (
                            ...
                            <>
                                <svg className="w-5 h-5 text-indigo-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
